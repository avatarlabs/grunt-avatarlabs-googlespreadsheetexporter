...
function _createObj(key, value, attributes) {
  ...
  if (attributes != '' || attributes != undefined) {
    ...
    _createAttributes(obj, attributes);
  }
  else {
    obj[key] = value;
  }
...
function _writeJson(data) {
  ...
  _.each(data, function (row, index) {
    ...
    */
    ...
    var child = row['1'], value = _.unescape(row['2']), attributes = row['3'], parent = row['4'];
    ...
    if (_.has(file, parent)) {
      ...
      //an array, push child.
      if (_.isArray(file[parent])) {
        file[parent].push(_createObj(child, value, attributes));
      }
